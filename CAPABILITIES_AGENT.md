...
| **Incoming email (IMAP)** | The agent wakes on new mail in a connected mailbox. IMAP/SMTP servers configured in Settings → Channels; supports threading and per-sender access control. | `docs/application/email_integration/email_integration.md`, `docs/application/email_integration/mail_servers.md` |
...
| **Cinna CLI** | **For developers building/iterating on an agent** — local workspace editing with continuous sync over a tunnel; credentials stay on the platform. Not a runtime/problem-solving channel. | `docs/application/cinna_cli_integration/cinna_cli_integration.md` |
