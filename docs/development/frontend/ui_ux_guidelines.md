...
**Reference:** `frontend/src/components/Agents/AgentHandovers.tsx` + `HandoverRow.tsx` + `AllHandoversSheet.tsx` (cap 5, Show all → Sheet, shared row component so card and Sheet cannot drift). `DesktopSessionsCard` ("App Sessions") is anti-pattern A4 until converted to this shape.
...
| A3 | **Expand-in-place editing** | `UserSettings/UserChannelsCard.tsx` (1056 lines): every row expands its full edit form, card grows page-wide | §2 card height, inline forms | Rows as P3; edit in a dialog/sheet |
| A4 | **Unbounded list in a card** | `UserSettings/DesktopSessionsCard.tsx` renders all sessions | §2 list inside a card | P5, 5 rows + Show all |
| A5 | **Every action visible on every row** | `Agents/AgentSchedulesCard.tsx` 5 icon buttons per row; `UserSettings/AICredentials.tsx` star + pencil + trash per row | §2 row actions | P3: Run + Logs inline, rest in menu; star as passive badge, toggle in menu |
