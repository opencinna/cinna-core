...
- That is **22** verdict codes in the current build (`grep -c "^CODE_" routing_reachability_service.py`), each with its own test in `routing_reachability_verdict_test.py` pinning both the code and its exact sentence; the test file has 29 test functions because several codes (e.g. the "route added/removed after the decision" pair, `CODE_NO_CANDIDATES_CHANNEL_SCOPE`'s two owners, and the near-miss-ranking behaviour) are covered by more than one scenario.
