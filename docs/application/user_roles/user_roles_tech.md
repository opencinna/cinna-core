...
- **Agent Integrations tab** (`frontend/src/components/Agents/AgentIntegrationsTab.tsx`) — when `isAgentUser`, renders only `McpConnectorsCardSimple` (the degraded MCP Connectors view); the full tab (direct MCP connectors, identity bindings, extra App MCP routes) is visible only to developers
