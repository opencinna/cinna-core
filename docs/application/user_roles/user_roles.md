...
| [Admin-Provisioned AI Credentials](../ai_credentials/admin_ai_credential_provisioning.md) | `ManagedAICredential.auto_provision_roles` selects on `User.role` at account creation. The per-credential role toggles on the **Company AI credentials** card, **Admin → Server Configuration → Access**, edits the same flag; the AI Credentials table (`/admin/ai-credentials`) shows it as an **Auto** column of role chips |
