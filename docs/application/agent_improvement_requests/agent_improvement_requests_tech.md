...
### `context` JSON shape (schema_version 2)
...
- `router_trigger` has no `*_updated_at` clock on `Agent` (only the three bidirectional prompts do), so its `updated_at` is always `null`.
...
- `_prompts_block(db, source_agent) -> dict` — the four prompt documents plus `agent_sdk_config` tool lists, with per-field divergence against `AgentBundleRevision` (see the `prompts` block above). Loses only the baseline if the revision lookup fails.
...
- **No migration for prompts/memory.** They live inside the existing `context` JSON column. `context.schema_version` goes `1 → 2` and archive `schema_version` goes `1 → 2`; pre-existing rows keep version 1 and are rendered without the new sections rather than backfilled — a backfill would read the *current* prompts and memory, which is exactly the live-read-through the feature forbids.
