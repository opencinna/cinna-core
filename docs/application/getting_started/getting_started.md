...
- **Disclaimer** — When a superuser has enabled the server-wide disclaimer, it appears before the Getting Started Modal for new users. The Getting Started Modal is deferred until the disclaimer is acknowledged — the two cannot appear simultaneously. See [Disclaimer](../server_configuration/disclaimer.md)
