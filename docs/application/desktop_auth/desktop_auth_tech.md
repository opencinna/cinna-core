...
- `frontend/src/components/UserSettings/DesktopSessionsCard.tsx` — "App Sessions" card: fetches `listDesktopClients()`, sorts most-recently-used first (never-used clients last), and renders through the shared `Common/PreviewList.tsx` primitive (5-row preview + "Show all (N)" + the four states — loading/error/empty/rows). Filename and export are unchanged from before the P5 rebuild
- `frontend/src/components/UserSettings/AppSessionRow.tsx` — one connected app as a compact row: platform-icon tile (`PlatformIcon`, macOS/Windows/Linux/iOS/Android, generic fallback), device name, `CLI link` badge (with `Tooltip`) when `origin === "cli_exchange"`, a ≤2-fact metadata line (`v{app_version}` and "Last used …" / "Connected …"), and a hover-revealed ghost `Unplug` icon button that opens a row-owned `AlertDialog` confirm before calling `revokeDesktopClient`. Rendered identically by the card and by the Sheet below, so the two hosts cannot drift
- `frontend/src/components/UserSettings/AllAppSessionsSheet.tsx` — "Show all (N)" destination: a full-height `Sheet` (no app-sessions route exists) rendering every connected client via `AppSessionRow`
- `frontend/src/components/Common/PreviewList.tsx` — shared P5 primitive (also adopted by `Agents/AgentHandovers.tsx`): sorted items in, capped preview + "Show all" link + the four list states out; not specific to app sessions
- `frontend/src/utils/appSessions.ts` — shared `APP_SESSIONS_QUERY_KEY` used by the card and the row's revoke mutation to invalidate the same cache entry
