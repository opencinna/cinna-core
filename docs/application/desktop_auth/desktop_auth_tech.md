...
- `backend/tests/api/cli/test_account_desktop_token.py` — Scenario tests for the CLI account-token exchange: lifecycle, client binding + origin laundering, auth matrix, audit events — and, unlike every other item in this list, one scenario asserting an **absence**: `test_desktop_token_exchange_is_not_developer_gated` demotes the user to `agent-user` (checking the demotion took) and asserts the exchange still succeeds. The endpoint is not role-gated, deliberately — see the ruling in `AccountCLIService.exchange_for_desktop_token`'s security position
