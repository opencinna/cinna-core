...
- `backend/app/api/routes/cli.py` — Phase 3 routes appended to the existing account CLI section:
  ...
  - `POST /api/v1/cli/account/agents/{agent_id}/rebuild-env` — `AccountCLIContextDep`; path `agent_id`; response `AccountRebuildEnvResult` (`{environment_id, status, status_message, was_running}`). `AgentService.assert_can_build` gate; wraps `EnvironmentService.rebuild_environment` (the same path as the UI rebuild menu item and the `/rebuild-env` session command); blocks for the whole rebuild — minutes, and the CLI client raises its own timeout to 1800 s for this call; maps `CanBuildError` (404/403), `AgentApiError` / `AgentEnvironmentError` (their `status_code`), `ValueError` → 400 (no active env). **Audited** (`CLI_ACCOUNT_ENV_REBUILT`). Status codes: 200 / 400 / 401 / 403 / 404.
    **Not a synonym for `restart-env`:** a restart re-runs the same image; a rebuild replaces the container's `/app/core` from the template, and is the only thing that adds routes a pre-feature container lacks (`adapter_unsupported` on the skills read, `status="unsupported"` on a plugin sync). `was_running` is read from `EnvironmentService.get_environment_status` **before** the rebuild — the rebuild restores the state it found, so a stopped environment comes back stopped and the CLI needs the flag to say why a successful rebuild left nothing answering.
    **Client-side, `--yes` skips only the cost-of-time confirmation.** In `run_agent_rebuild_env` (`cinna-cli`, `src/cinna/account.py`) the D2 unsynced-changes guard runs *before* the `yes` check and is not gated on it, so a dirty workspace still gets "Rebuild anyway?" with `default=False` — under `--no-input` that default aborts, and with no terminal Click aborts. Same shape as `run_agent_restart_env`, which has no `--yes` at all. `--yes` is therefore not a general non-interactive switch; scripts `cinna sync push --agent <name>` first.
...
- `backend/app/models/events/security_event.py`:
  ...
  - `CLI_ACCOUNT_ENV_REBUILT = "CLI_ACCOUNT_ENV_REBUILT"` — written on `agent rebuild-env`; `severity="medium"`, `details={environment_id, was_running, ip}`, `agent_id` = target. A separate constant from the restart, on the same grounds it is a separate verb: rebuilding replaces `/app/core` from the template and recreates the container, which is a bigger act than bouncing it.
...
| `POST` | `/api/v1/cli/account/agents/{agent_id}/restart-env` | 200 / 400 / 401 / 403 / 404 | Restart the agent env — re-runs the same image (`can_build`-gated, audited); response `AccountRestartEnvResult` |
| `POST` | `/api/v1/cli/account/agents/{agent_id}/rebuild-env` | 200 / 400 / 401 / 403 / 404 | Rebuild the agent env — replaces `/app/core` from the template, the only way a pre-feature container gains new routes (`can_build`-gated, audited, blocks for minutes); response `AccountRebuildEnvResult` (adds `was_running`) |
