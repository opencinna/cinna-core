...
| `CINNA_CLI_VERSION` | `backend/app/core/config.py` | `"0.4.0"` | Pinned cinna-cli version served by `/sync-runtime` and by the `/.well-known/cinna-desktop` `local_dev` block. A pin, not "latest" — bump when a newer CLI release is verified against this instance |
