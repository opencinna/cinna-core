...
This entry point moved here from the agent's Integrations tab ("Identity MCP Server Integration" is gone from that dialog) as part of the same phase that removed App MCP's route-based creation flow — see [Identity MCP Server](../identity_mcp_server/identity_mcp_server.md).
...
- **[Identity MCP Server](../identity_mcp_server/identity_mcp_server.md)** — `IdentityAgentBinding.prompt_examples` is aggregated and prefixed by `IdentityCandidateProvider` when building Stage 1 candidates for a caller
