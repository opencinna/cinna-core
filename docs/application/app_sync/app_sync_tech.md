...
Short-lived blind relay for device pairing (commit-then-reveal protocol). The server stores and forwards opaque blobs — it never verifies the commitment and never sees the UMK.

| Column | Type | Constraints | Notes |
|--------|------|-------------|-------|
| `id` | UUID | PK | Used by sealer-facing inbox endpoints |
...
| `commitment` | TEXT | NOT NULL | `blake2b(pubkey_J ‖ nonce_J, 32 bytes)`, base64; sent by joiner at `start`; verified by sealer client-side (relay never checks it) |
| `device_label` | VARCHAR(128) | nullable | Optional label provided by the joining device |
| `sealer_nonce` | TEXT | nullable | Base64 nonce generated by the sealer; `NULL` until `POST /pairing/inbox/{id}/sealer-nonce` |
| `joiner_nonce` | TEXT | nullable | Base64 nonce revealed by the joiner; `NULL` until `POST /pairing/{code}/reveal` |
| `sealed_umk` | TEXT | nullable | UMK sealed to `new_device_pubkey` by the sealer; `NULL` until `POST /pairing/inbox/{id}/complete` |
| `status` | VARCHAR(24) | NOT NULL, default `pending` | `pending` → `sealer_nonce_set` → `revealed` → `completed` → `consumed`; or `expired` at any non-terminal state |
...
Pairing schemas are in `app_sync_pairing.py`:

```python
PairingStartRequest         # Body for POST /pairing/start
    new_device_pubkey: str
    commitment: str          # blake2b(pubkey_J ‖ nonce_J, 32 bytes), base64
    device_label: str | None

PairingStartResponse        # Response for POST /pairing/start
    pairing_code: str
    expires_at: datetime

PairingStatusPublic         # Response for GET /pairing/{code}
    status: str              # pending | sealer_nonce_set | revealed | completed | consumed | expired
    sealer_nonce: str | None # Null until sealer posts it
    sealed_umk: str | None   # Present only when status == completed (single-use; row becomes consumed)
    expires_at: datetime

PairingRevealRequest        # Body for POST /pairing/{code}/reveal
    joiner_nonce: str        # Reveals the nonce hidden in the commitment

PairingInboxItem            # One entry in GET /pairing/inbox response
    id: UUID
    device_label: str | None
    status: str
    expires_at: datetime

PairingInboxDetail          # Response for GET /pairing/inbox/{id}
    new_device_pubkey: str
    commitment: str
    sealer_nonce: str | None
    joiner_nonce: str | None
    status: str
    expires_at: datetime

PairingSealerNonceRequest   # Body for POST /pairing/inbox/{id}/sealer-nonce
    sealer_nonce: str

PairingCompleteRequest      # Body for POST /pairing/inbox/{id}/complete (reused)
    sealed_umk: str
```
...
Inbox routes are declared before `/pairing/{code}` in the router so a base64url code cannot be path-matched as the literal string `inbox` or as a UUID.

**Joiner-facing (keyed by secret `code`):**

| Method | Path | Body | Response | Description |
|--------|------|------|----------|-------------|
| `POST` | `/pairing/start` | `PairingStartRequest` | `PairingStartResponse` | Joining device: creates relay row (`status=pending`) → returns `pairing_code` + `expires_at`. Body now includes `commitment`. |
| `GET` | `/pairing/{code}` | — | `PairingStatusPublic` | Joining device polls. Response includes `sealer_nonce` (null until set) and `sealed_umk` (present only at `completed`). Retrieving `sealed_umk` marks row `consumed` (single-use). `404` if not found; `410` if expired |
| `POST` | `/pairing/{code}/reveal` | `PairingRevealRequest` | `Message` | Joining device reveals `joiner_nonce` after seeing `sealer_nonce`. `409` if status is not `sealer_nonce_set`; `410` if expired |

**Sealer-facing (keyed by row `id`, discovered via inbox):**

| Method | Path | Body | Response | Description |
|--------|------|------|----------|-------------|
| `GET` | `/pairing/inbox` | — | `list[PairingInboxItem]` | Lists the caller's own non-terminal pairing rows (`id, device_label, status, expires_at`). No pubkeys, nonces, or `sealed_umk` — discovery metadata only |
| `GET` | `/pairing/inbox/{id}` | — | `PairingInboxDetail` | Full detail for one of the caller's own rows: `new_device_pubkey, commitment, sealer_nonce, joiner_nonce, status, expires_at`. No `sealed_umk`. `404` if not found or owned by another user |
| `POST` | `/pairing/inbox/{id}/sealer-nonce` | `PairingSealerNonceRequest` | `Message` | Sealer posts its nonce. `pending → sealer_nonce_set`. `409` if status is not `pending`; `410` if expired |
| `POST` | `/pairing/inbox/{id}/complete` | `PairingCompleteRequest` | `Message` | Sealer posts `sealed_umk` after verifying commitment and SAS. `revealed → completed`. `409` if status is not `revealed`; `410` if expired |

> The old `POST /pairing/{code}/complete` (code-keyed sealer complete) has been removed. The sealer now completes by row `id` via `POST /pairing/inbox/{id}/complete`.
...
```python
AppSyncService.pairing_start(session, user, *, new_device_pubkey, commitment, device_label) -> PairingStartResponse
    # Generates a 24-byte URL-safe random code, stores SHA-256 hash, stores
    # commitment, sets TTL. Row created with status = "pending".

AppSyncService.pairing_get(session, user, *, code) -> PairingStatusPublic
    # Loads by SHA-256(code); checks expiry → 410. Returns sealer_nonce (null until
    # set) and status. If status == "completed" and sealed_umk is present: clears
    # sealed_umk, sets status = "consumed" (single-use delivery), reports
    # status = "completed" to this caller.

AppSyncService.pairing_reveal(session, user, *, code, joiner_nonce) -> None
    # Loads by SHA-256(code); validates status == "sealer_nonce_set" → 409 if not;
    # stores joiner_nonce; sets status = "revealed".

AppSyncService.pairing_inbox(session, user) -> list[PairingInboxItem]
    # Lists the caller's own non-terminal pairing rows (status not in
    # {completed, consumed, expired}). Returns id, device_label, status,
    # expires_at. No secrets.

AppSyncService.pairing_inbox_get(session, user, *, pairing_id) -> PairingInboxDetail
    # Loads by id; verifies user_id == user.id → 404 if not.
    # Returns new_device_pubkey, commitment, sealer_nonce, joiner_nonce,
    # status, expires_at. No sealed_umk.

AppSyncService.pairing_set_sealer_nonce(session, user, *, pairing_id, sealer_nonce) -> None
    # Loads by id; verifies ownership; validates status == "pending" → 409 if not;
    # stores sealer_nonce; sets status = "sealer_nonce_set".

AppSyncService.pairing_complete_by_id(session, user, *, pairing_id, sealed_umk) -> None
    # Loads by id; verifies ownership; validates status == "revealed" → 409 if not;
...
### Pairing commitment fields

**File:** `backend/app/alembic/versions/a3c9e1f7b204_app_sync_pairing_commitment_fields.py`

- **Revision:** `a3c9e1f7b204`
- **Depends on:** `d54391bd8cf2` (initial tables)

**Upgrade** adds three columns to `app_sync_pairing` and extends the `status` value set:
- `commitment TEXT NOT NULL` — joiner's blake2b commitment, stored verbatim
- `sealer_nonce TEXT` (nullable) — sealer's nonce, populated at `sealer_nonce_set`
- `joiner_nonce TEXT` (nullable) — joiner's revealed nonce, populated at `revealed`
- `status` extended to accept `sealer_nonce_set` and `revealed` in addition to the original `pending`, `completed`, `consumed`, `expired`

Note: the `status` column type is `VARCHAR` (not a DB enum), so adding new string values requires no DDL type change — only application-level validation is updated.
