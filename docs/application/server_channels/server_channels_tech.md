...
- `backend/app/services/server_channels/channel_inbound_service.py` — `ChannelInboundService`: the full webhook → routing → session pipeline
...
In-memory, process-local capture of recent channel traffic, feeding the admin debug panel — deliberately not persisted. Documented separately: [Channel Debug Monitor](channel_debug_monitor.md) / [tech](channel_debug_monitor_tech.md).
...
- `_route_installed` (Pass 1) — calls `AppMCPRoutingService.route_message`, then applies the ownership filter: rejects `result.is_identity` outright and requires `agent.owner_id == user.id`
- `_route_catalog` (Pass 2) — builds the candidate list from `ServerAutoInstallBundle`, filtering on: not already installed by this user, `CatalogService.user_can_install(db, bundle, user)`, and a non-empty `router_trigger_prompt` on the latest revision; classifies via `AIFunctionsService.route_to_agent`
- `_in_thread` — offloads both routing passes' blocking LLM HTTP calls via `anyio.to_thread.run_sync`; the thread targets open their **own** DB session and return plain ids, never closing over the caller's session (a cancelled task can't interrupt a running thread)
...
- **Known logging gap (deferred, not fixed here):** Pass-1 routing calls into the pre-existing `app_agent_router.py` / `app_mcp_routing_service.py`, which log inbound message text at INFO level. That logging predates this feature and was written for internal App MCP traffic; through this feature the same code path now also carries externally-sourced, non-platform-user text. Not touched here because the logger is shared with the live App MCP feature.
