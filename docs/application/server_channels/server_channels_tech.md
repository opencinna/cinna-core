...
- **Message-text logging gap — closed.** Pass-1 routing calls into `app_agent_router.py` / `app_mcp_routing_service.py`, which used to log inbound message text at INFO level — logging that predates this feature and was written for internal App MCP traffic, but through this feature the same code path also carries externally-sourced, non-platform-user text. The Auto Routing Tuning work (Phase 1) downgraded those lines to `debug` and removed the code comment that flagged this as a deferred gap; the durable `routing_decision` trace is the intended replacement observability surface (superuser-only, TTL'd — see `docs/plans/auto_routing_tuning_plan.md` §5), not an INFO log line.
