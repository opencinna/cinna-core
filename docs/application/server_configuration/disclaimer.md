...
- **Singleton Config** — One `ServerConfig` row holds all settings. There is no per-user or per-agent configuration; the same disclaimer applies to everyone on the instance. The row also carries the unrelated `local_agent_kit_enabled` instance switch (see [Local Agent Kit](../local_agent_kit/local_agent_kit.md)) — its card lives on the same Interface tab, next to the Disclaimer card, and shares this doc's `["serverConfig"]` query key and update endpoint — the six [access-policy](access_policy.md) columns, and `landing_markdown`, the [public landing page](landing_page.md)'s welcome copy (edited from the **Access** tab). Updating one field never touches the other's semantics: neither `local_agent_kit_enabled` nor `landing_markdown` ever bumps `disclaimer_version`, and disclaimer edits never affect them. `landing_markdown` is a second Markdown column on this row and is **not** a second disclaimer: it is never acknowledged, it is served to anonymous callers, and it is refused if it looks like raw HTML
...
- **Public Landing Page** — The other admin-authored Markdown column on the same row, edited from the **Access** tab and rendered to anonymous visitors on `/start`. Same endpoint and query key, opposite audience: the disclaimer is shown to signed-in users and acknowledged; the landing copy is shown to everyone and never is. See [Public Landing Page](landing_page.md)
