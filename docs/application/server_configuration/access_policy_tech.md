...
- `frontend/src/components/Admin/AccessPolicy/CompanyAiCredentialsCard.tsx` + `CompanyAiCredentialRow.tsx` — one row per **AI Provider** with a three-segment role `ToggleGroup` editing `AIProvider.auto_provision_roles`, capped at 5 rows; see [AI Providers — tech](../ai_credentials/admin_ai_credential_provisioning_tech.md)
...
| `CompanyAiCredentialsCard` (**Company AI providers**) | `PreviewList` of provider rows (name, with the type and the kind on the metadata line — "OpenAI · Per-user keys" — and a **User · Dev · Admin** `ToggleGroup type="multiple"` as the row's one control), capped at 5 rows (granted-first then alphabetical), link to `/admin/ai-credentials#providers` | **Not a `ServerConfig` column.** Each toggle is a `PATCH /admin/ai-providers/{id}` carrying only `auto_provision_roles` — the rule lives on the Provider, so the card moved with it — and it shares the Providers query key (`["admin", "ai-providers"]`) and renders the `409 auto_provision_conflict` inline |
