...
- `backend/app/services/users/account_provisioning_service.py` - `AccountProvisioningService.on_account_created` / `provision_explicit` / `on_account_deactivated`. Two entry points, one shared body (`_provision`) and one shared outer net (`_guarded`). See [Admin-Provisioned AI Credentials — tech](../ai_credentials/admin_ai_credential_provisioning_tech.md#accountprovisioningservice-servicesusersaccount_provisioning_servicepy)
