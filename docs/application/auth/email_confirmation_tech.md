...
- `recover_password`: password recovery is **never** gated by `email_confirmed`. A 300-second per-user cooldown (`last_password_recovery_email_sent_at`) rate-limits repeated sends; when in cooldown, the send is skipped silently so the public response stays generic. **The 404 for an unknown address is gone** — the route now always answers `200` with the same body for a known and an unknown address, and every reason not to send (including a deactivated account and an unclaimed invited account whose invitation is no longer pending) is a silent no-op. The method returns `bool` for tests and internal callers; the route deliberately does not project it. See [Authentication — tech](auth_tech.md#login-and-recovery-timing) for the residual timing channel.
- `mark_confirmed` gained a caller: `InvitationService.accept_with_password` calls it once the new password and `accepted_at` have committed, so accepting an invitation confirms the address. The call is guarded — a failure there leaves the address unconfirmed and logs, rather than 500-ing someone whose password is already set and whose invitation is already terminal.
