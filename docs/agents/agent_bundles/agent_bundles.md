...
- **Workspace separation on update** — `replace_bundle_content` overwrites only bundle-owned folders (`scripts/`, `docs/`, `knowledge/`, `files/`, `workspace_requirements.txt`, `workspace_system_packages.txt`); `app-data/` and `credentials/` are never touched; schedules are merged separately via behavioral signature (not blindly replaced)
