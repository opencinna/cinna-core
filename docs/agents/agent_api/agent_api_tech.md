...
- `frontend/src/components/Common/AgentSelectorDialog.tsx` — shared agent picker (Bot badge + colour preset) reused by the connect dialog and the key mint dialog. Since the skills work it exports **two** things: `AgentSelectorList` (search box + the colour-preset agent cloud, no `Dialog`) and `AgentSelectorDialog` (that list inside a `Dialog`, closing on select). The list takes `mode` — `multi` (default, the cloud stays open and selected pills carry an outline) or `single` (once an agent is chosen the cloud **collapses to that agent's `AgentBadge`** with Change / clear beside it). The split exists so a *form* can hold the same picker: a dialog-based picker cannot be one of a form dialog's fields (§2 "Disclosure depth"), and the alternative — a plain `SearchableSelect` of names — picked agents by a different vocabulary from every other surface. The dialog's own props omit `mode`; it closes on select, so a collapsed state would never be seen
