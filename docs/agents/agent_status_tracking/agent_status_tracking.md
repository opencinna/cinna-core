...
3. `AgentStatusService.handle_post_action_event` is registered against **seven** events (the six above plus `ENVIRONMENT_ACTIVATED` — see flow 7 below). It reads `environment_id` from the event meta and calls `refresh_after_action(env)`.
...
### 7. Pull on environment activation (gap fix)
