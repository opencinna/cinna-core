...
- `backend/app/api/routes/agents.py`
  ...
  - `PATCH /api/v1/agents/{id}/router-trigger-prompt` - Owner-only update of `Agent.router_trigger_prompt`. Accessible to `agent-user` accounts (no `require_developer` gate). Body: `RouterTriggerPromptUpdate {router_trigger_prompt: str | None}`. Also propagates the new value to the auto-managed `AppAgentRoute` via `AppAgentRouteService.sync_router_trigger_prompt_from_agent`, which creates that route when the install has none yet. The propagation is best-effort: its `except` calls `session.rollback()` before logging, because a failed statement aborts the transaction and the `_agent_to_public` projection immediately below would otherwise raise `InFailedSqlTransaction` and turn the swallowed warning into a 500 (same guard in `AgentService.update_agent`). Returns `AgentPublic`
