...
| agent_bundles | Desktop-app-style bundle / install model — agents published as versioned bundles, installed by other users, push-updated; bundle ID format, visibility/grants, revisions, publisher working install; auto-creates an App MCP route at install time from the revision's router trigger prompt | [business logic](agents/agent_bundles/agent_bundles.md) \| [tech](agents/agent_bundles/agent_bundles_tech.md) |
| agent_app_data | Per-user, per-bundle persistent App Data volume (`/app/workspace/app-data`) keyed by `(user_id, bundle_id, catalog_type)` — survives uninstall/reinstall, Settings tab management, manual wipe on orphaned volumes | [business logic](agents/agent_app_data/agent_app_data.md) \| [tech](agents/agent_app_data/agent_app_data_tech.md) |
