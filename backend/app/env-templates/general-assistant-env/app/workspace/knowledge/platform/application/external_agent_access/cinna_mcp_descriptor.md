...
| **Tool name slug** | Stable, lowercase, `[a-z0-9_]+` identifier derived from the agent or route name. Collision-free across a user's full reachable set via deterministic suffixing |
...
When a client fetches the extended agent card from any external A2A endpoint
(`GET /api/v1/external/a2a/agent/{id}/`, `/route/{id}/`, or `/identity/{id}/`),
the `capabilities.extensions` array includes an entry with `uri = "urn:cinna:mcp"`.
...
- For `target_type="agent"` and `target_type="app_mcp_route"` targets the `mcp` field is populated.
- For `target_type="identity"` the `mcp` field is `null` — same reason as above.
...
| `tool_name` | Slugified agent or route name, deconflicted | `[a-z0-9_]+`, capped at 48 chars |
| `display_name` | `Agent.name` or route name | Human-readable label in the desktop tool list |
| `description` | Base + `router_trigger_prompt` folded in | For route targets: base + route `trigger_prompt`; for personal agents without a trigger prompt: base + `Agent.description`; for personal agents without either: base only |
...
There is no `slug` field on `Agent`. The slug is generated at response time from the agent or route name:
...
Collision detection runs across the user's **full reachable set** (all three sections of the discovery response) in a single pass. When two or more targets produce the same base slug, every colliding entry is suffixed with the first 8 hex characters of its `agent.id` (no hyphens) — making the result deterministic regardless of ordering or re-request timing.
...
| `app_mcp_route` (shared) | Route name (`AppAgentRoute.name` or fallback `agent_name`) | Route `trigger_prompt` | Route name |
| `identity` | N/A | N/A | N/A — `mcp = null` |

For route targets the descriptor reflects the identity the caller sees (the shared-route name and trigger), not the raw underlying agent. This keeps the tool name and description consistent with what the caller experiences in the A2A card.
...
- The descriptor rides on the **extended (authenticated) agent card** built by `build_agent_card`, so it appears on the authenticated extended card of **both** the internal (`/api/v1/a2a/{id}/`) and external (`/api/v1/external/a2a/...`) A2A surfaces. It is **not** present on the minimal public card returned to unauthenticated callers. The route-identity overrides (slug / display name / description from the route) and the discovery `mcp` mirror (`/api/v1/external/agents`) are external-surface only; the internal card always uses the agent-name slug. Cinna Desktop consumes the descriptor exclusively from the external surface.
...
- **[App MCP Server](../app_mcp_server/app_mcp_server.md)** — `AppAgentRoute` and `EffectiveRoute` provide the route name and trigger prompt used as the descriptor identity for shared-route targets
