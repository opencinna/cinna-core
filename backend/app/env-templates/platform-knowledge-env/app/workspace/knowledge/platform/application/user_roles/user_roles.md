...
2. `User.role` is set to `ServerConfig.default_user_role` (default: `agent-user`; a superuser can switch it to `agent-developer` on the **New user defaults** card, **Admin → Server Configuration → Access**)
3. If a superuser has marked any **AI Provider** with that role under `auto_provision_roles`, the new account already holds a key from it — set as their default and, when the record wires SDK defaults, pinned to the admin's chosen model. Nothing is asked of the user. See [Admin-Provisioned AI Credentials](../ai_credentials/admin_ai_credential_provisioning.md#auto-provisioning-at-account-creation)
...
- **Default on signup** — the role assigned to a new non-superuser account comes from `ServerConfig.default_user_role`, edited by a superuser on **Admin → Server Configuration → Access** and read through `AccessPolicyService.default_role`. Allowed values: `agent-user` (default) and `agent-developer`; `admin` is rejected at validation to preserve the `role ⇔ is_superuser` invariant, and an out-of-range stored value degrades to `agent-user`. The `DEFAULT_USER_ROLE` env setting is **retired**: it seeds the column on a brand new instance (and once, by migration, on an upgrading one) and nothing reads it for a live decision — a startup warning names the admin page when it is still set. The setting is useful for standing up a server where all self-signup accounts are developers from day one. The superuser exception is unaffected: any user created with `is_superuser = true` always gets `role = 'admin'` regardless. See [Access Policy](../server_configuration/access_policy.md)
...
- **The role selects which company AI keys a new account receives.** `AccountProvisioningService.on_account_created` grants every **`AIProvider`** whose `auto_provision_roles` contains the new account's role, through the managed credential that Provider owns. `admin` is a valid choice there — an instance where every employee is an administrator is a normal small-team shape.
- **A role change never re-provisions.** Promotion and demotion are pure permission changes: they grant no key and revoke none. An admin who wants existing accounts to receive a credential uses **Apply to existing users** on the AI Credentials page (`/admin/ai-credentials`). This is deliberate — a promotion silently handing out a company API key would be a surprise grant.
...
- **Integrations tab (agent-user)**: only the **MCP Connectors** card is rendered, using `McpConnectorsCardSimple`. Since Phase 5 of the channels & identity unification the card has **no query and no mutation** — it takes `routerTriggerPrompt` as a prop and is purely explanatory. There is no App MCP route and no per-user toggle: every agent the user owns is reachable from their MCP Server URL as soon as it has a trigger prompt. No "New" dialog, no Direct MCP connector option, no user-share multi-select. Because this card is the role's only exposure to App MCP routing, its copy carries the whole explanation — a static "Available in external MCP clients" heading whose body says outright that "there is nothing to switch on", the trigger prompt framed as a quoted routing rule under "When this agent gets picked", and a footer noting that this affects MCP clients only (chat, schedules, and other integrations keep working) plus a link to `/settings#channels` for the MCP Server URL. When the prompt is blank, a dashed-border hint directs the user to set a Trigger Prompt on the Configuration tab — doing so makes the agent reachable immediately; it creates no route
...
- **Invite wizard (`Invite user`, the page header's primary action)**: step 1 carries the same three-value role dropdown, fed from the shared `USER_ROLE_OPTIONS`, so the role a new account gets is chosen at invite time rather than corrected afterwards. Step 2 then pre-ticks exactly the **AI providers** that role's `auto_provision_roles` would have granted anyway.
  - **`role="admin"` implies `is_superuser`.** The request carries no `is_superuser` field at all: `InvitationService.invite` derives it as `role == UserRole.ADMIN.value` and passes both to the chokepoint. That is not defensive tidiness — `create_account` treats `is_superuser=True` as pinning the role to `admin`, but it will also *accept* `role="admin", is_superuser=False` and produce a row that breaks the role ⇔ superuser invariant. Deriving is what stops the wizard being able to ask for that. An invited `admin` is therefore also auto-confirmed, by the chokepoint's existing superuser rule.
  - The same derivation is applied on the retry path that re-arms an interrupted invite's account, for the same reason.
  - The **Status** column and the row menu's invitation actions are role-independent — see [Auth → Invitations](../auth/auth.md#invitations).
...
| [Access Policy](../server_configuration/access_policy.md) | Owns `ServerConfig.default_user_role`, the source of truth for the role a new non-superuser account receives. Superusers edit it on the Access tab of Server Configuration. An **invitation** overrides it: the wizard states the role explicitly, so the policy default only applies to arrivals that state nothing |
| [AI Providers & Admin-Provisioned AI Credentials](../ai_credentials/admin_ai_credential_provisioning.md) | `AIProvider.auto_provision_roles` selects on `User.role` at account creation. The per-provider role toggles on the **Company AI providers** card, **Admin → Server Configuration → Access**, edit the same flag; the Providers tab (`/admin/ai-credentials#providers`) shows the rule on each provider's row |
