...
- `backend/app/env-templates/platform-knowledge-env/docker-compose.template.yml`
...
- **[Multi-Image Environments](./agent_multi_image_environments.md)** — All three environment templates (`general-env`, `python-env-advanced`, `platform-knowledge-env`) include the `claude_sessions` volume mount. Template selection does not affect session persistence behavior.
