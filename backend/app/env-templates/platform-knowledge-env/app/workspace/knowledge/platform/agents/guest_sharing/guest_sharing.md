...
- **Agent Management** — The **Guest Share Links** card lives on the agent's Integrations tab alongside A2A tokens, Agent REST API, MCP Connectors, Webhooks, Webapp Share, Local Dev, and GIT Versioning (there is no Email card — per-agent Email Integration was deleted in Phase 4 of the channels & identity unification). Visible to owner only. See [Agent Management](../../application/agent_management/agent_management.md)
