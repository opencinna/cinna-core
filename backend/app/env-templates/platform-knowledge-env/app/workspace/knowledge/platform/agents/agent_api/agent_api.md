...
**Global Credentials view.** `agent_api` connections appear under the **Automatic Credentials** tab in `/credentials`. Tab membership is a server-computed `category` field — `agent_api` and `mcp_provider` credentials both land in `"automatic"` (see [Credential Sharing — Categorization rules](../agent_credentials/credential_sharing.md#credentials-page-filter-tabs-and-category-assignment)). Their detail page lets you edit **name and notes** (the proxy token is still never shown), keeps the **Sharing** card, and **omits the Template-sharing** card — a connection has no user-fillable private fields, so it can never be template-provided.
