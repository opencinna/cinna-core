...
#### The installer is told why: `publisher_credential_unshareable`

`InstallReadinessGate` gained a fourth `GateMissingReason`, `publisher_credential_unshareable`. It looks for an `AICredentialShare` to the installer **first**, and asks `is_shareable` only when there is none.
...
Only (3) is about policy. The earlier order asked (3) first and reported it as though it were (1) and (2), telling an installer whose agent was running that their publisher's credential "cannot be shared". The gate's list is *what the install is missing*, and a credential the installer can already use is not missing — so when a share exists the gate now reports **nothing at all**. Fact (3) is told to the publisher, at publish time, where somebody can act on it: see [Told at publish time](#told-at-publish-time-publish_notices).

With no share, the policy question is real, and reporting `publisher_credential_unshared` would tell the publisher to do something the server will refuse forever — hence the fourth reason.
...
It maps to the `publisher_broken` status like the other publisher-side reasons, but takes its own copy in both renderers, because the generic wording asks the publisher to *restore access* and there is no access to restore:
...
**`GateMissingReason` moved to `backend/app/models/bundles/catalog.py`.** It had been spelled out in the gate service *and* again in the response model, and adding the fourth reason made the gate produce a value the response model rejected — a validation error at the boundary between two copies of one list, surfacing as a **500**. One list, declared once, imported by the other. See [Agent Bundles — tech](../../agents/agent_bundles/agent_bundles_tech.md#installreadinessgate-phase-4).

#### Told at publish time (`publish_notices`)

**The gate tells the wrong person.** It reports an unshareable publisher credential to the *installer*, on their setup screen, after they have installed something that does not work the way its publisher believed. The person who wired the credential and the person who is told about it are different people at different moments, and the one who could act on it never saw it.
...
14. **An unshareable publisher credential blocks every message on an install that actually runs, and the installer cannot clear it. Escalated, deliberately unfixed.** When a bundle's publisher AI credential is not shareable *and* no existing share reaches the installer, two things happen that do not agree. The environment falls back to the installer's own default (`InstallService._linkable_publisher_ai_credential` returns `None`, so the install can run), while `InstallReadinessGate._scan_ai_credentials` still emits `publisher_credential_unshareable` (`install_readiness_gate.py:290`) — a `publisher_broken` verdict that short-circuits **every inbound message** on chat, MCP, A2A and session webhooks before any LLM call.

    It is **unclearable by the installer**. The scan reads `bundle.publisher_ai_credential_*_id`, and no action available to an installer changes a bundle FK. The user-facing copy (`install_readiness_gate.py:392`) says *"in the meantime you can supply your own from the agent's Credentials tab"*, which is wrong twice: supplying a credential there does not clear the gate, and it is **the wrong screen** — the agent's Credentials tab has no AI-credential control at all, and the AI fallback the environment already took resolves from the user's default in **Settings → AI Credentials** (or a per-environment pin on the agent's **Environments** tab, via `EnvironmentConfigForm`). The only per-mode AI credential control that exists on the Bundle tab is the *publisher's*, in `CredentialProvisioningSection`.

    Pre-existing from the fourth pass. Escalated in the fifth as a **product question — should an unshareable publisher credential block the install at all, given the environment has already fallen back to a working credential?** — and left unfixed pending that decision, rather than patched into a shape that presumes the answer.
