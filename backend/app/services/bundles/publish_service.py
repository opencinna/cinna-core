...
# The plugins tree is snapshotted like a bundle folder (snapshot dir name ->
# env-relative path) but with special handling: the publisher's
# ``settings.json`` / ``manifest.json`` are NOT copied — they are per-consumer,
# regenerated by the container install routine from each install's own manifest.
_BUNDLE_PLUGINS_SNAPSHOT_DIR = "plugins"
_BUNDLE_PLUGINS_ENV_REL = "app/workspace/plugins"
# Top-level plugin-dir files excluded from snapshot + seed (consumer-derived).
...
class PublishService:
    ...
    async def _publish_locked(
        ...
        *,
        ...
        try:
            ...
            # revision row. The plugin files are already in tmp_dir via
            # _copy_bundle_tree.
    ...
    def _copy_bundle_tree(
        ...
        # Plugins tree: copy publisher plugin file trees but exclude the
        # per-consumer derived files (settings.json / manifest.json). Always
        # create the snapshot dir so seeding stays consistent even when the
        # publisher has no plugins.
        plugins_dest = dest / _BUNDLE_PLUGINS_SNAPSHOT_DIR
        plugins_dest.mkdir(parents=True, exist_ok=True, mode=0o755)
        if env_workspace_root is not None:
            plugins_src = env_workspace_root / _BUNDLE_PLUGINS_ENV_REL
            if plugins_src.exists() and plugins_src.is_dir():
                PublishService._copy_plugins_tree(plugins_src, plugins_dest)
    ...
    def _collect_plugin_specs(session: Session, install: Agent) -> list[dict]:
        ...
        where the files live in the revision tree. The plugin *files* are copied
        by ``_copy_bundle_tree``; this captures only coordinates/flags.
