...
For each surface answer R1–R22 of guideline §8 with **yes / no / n/a** and a `file:line` (or screenshot name) as evidence. Count blocks, rows, inline actions, disclosure levels — write the numbers down. Compare the component's skeleton with the reference pattern's skeleton (R12) and with the spec (R13).

R17–R22 are the **mechanism gates**. R17–R19 were added 2026-09-09 after a PASSed surface collected fourteen user corrections; R20–R21 were added the same day after five 9–10/10 surfaces still produced five rule gaps, every one of them a rule that existed but could not be reached from where the author stood; R22 came hours later, from a second user round on those same PASSed surfaces (guideline §10). They cannot be judged from composition alone, so grep for them explicitly in every touched file:
...
- Two rows of same-styled pills where one navigates and the other filters, or a segmented on-state painted `bg-background` / `bg-accent` instead of `bg-primary`; a form field that picks one entity by rendering every candidate inline, or behind a nested `Dialog`, or that puts a **Change** button beside the value it already shows; a label that changes its verb phrase with the reader's history (`Add to another agent`) → R22 *no*.

### 4. Score and verdict
Score = 10 − (number of *no* among R1–R10); *n/a* counts as satisfied. R11–R22 are gates: a *no* is a finding that blocks PASS but does not change the score. Verdict per surface:
- **PASS** — score ≥ 8, none of R4 / R5 / R7 / R8 is *no*, and no R11–R22 finding is open.
...
- Do not review code quality, accessibility internals, or backend behaviour here. The R17–R22 greps are the exception: they are composition defects that only show in code (a timezone, a scrollbar, an empty string), so you look for them.
