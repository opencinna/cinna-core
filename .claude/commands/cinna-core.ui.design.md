...
Per surface: loading / empty / error / pending copy (guideline §6); components from §5 and any new primitive to add (name it, say which existing file it is extracted from); verification mode per §9 — `checklist` or `checklist + screenshots` with the exact URLs and click paths to capture.
...
### S1 — <name>
- **Intent:** …
- **Layout:** … (skeleton by reference: `<file>` lines …; what differs)
- **Rows / blocks:** …
- **Actions:** primary inline …; menu …; destructive …
- **Interaction model:** auto-save | form
- **States:** loading …; empty …; error …; pending …
- **Components:** …; new primitive: …
