...
- Mechanism rules, each a user-caught bug once (guideline §2 rows dated 2026-09-09 and 2026-09-09b, gates R17–R22):
  ...
  - **Badges:** ≤ 2 on a title line, and the second is an identity fact (version) **or** an origin fact (author, or the word standing in for it) — never a third of either. Before adding one: does another source's row spend this slot on something else (then it is the same slot), and would a user scan a list for this word (if not, it is a `RowFlag`)? If you find yourself writing a comment that argues with the guidelines, the rule is wrong — say so in your report instead of overriding it in a comment.
  - **Absent facts:** render **nothing** on a row. No "No version" / "Not set" / "None" chip — that is the `Active` failure, and it lands on every row in a young list. State the absence in the detail dialog's fact list, and prompt for it on the control that supplies it.
  - **Scrolling containers:** a capped `overflow-y-auto` gets `pr-2`, or the rightmost flag of every row sits under the scrollbar.
  - **Choice controls:** one-of-n is `role="radiogroup"` + `role="radio"` + `aria-checked`; each-of-n is `aria-pressed`. **Hand-rolled buttons included** — this is about the semantics, not about Radix. A control that unmounts on activation must move focus somewhere deliberate.
  - **Page toolbars:** navigation and filtering never share a control vocabulary. Sections are a segmented control on one bordered track with a `bg-primary` on-state (never `bg-background`, which is darker than the track on the dark skin, and never `bg-accent`, the hover colour); the subset filter is one `Filters` menu button carrying the active subset.
  - **Choosing one entity in a form:** a trigger + `Popover`, never an inline list of every candidate and never a second `Dialog`. The value sits on the trigger in its product-wide vocabulary (an `AgentBadge`, not a name), the trigger is how it is changed — no **Change** button beside it — and an `X` clears an optional field.
  - **Labels:** a control's label does not change its verb phrase with the reader's history ("Add to agent", never "Add to another agent"). The fact belongs in a line on the surface, not in the button.
  - **Fact labels:** the label is a noun, the value carries its unit ("Catalog installs" / "3 people"), never a predicate and never a bare number whose unit lives only in the label.
...
3. **Walk guideline §8 yourself** for each surface (R1–R22) and write the counts into your report: blocks per card, rows shown, inline actions per row, menu items, disclosure depth. For R17–R22 grep your touched files for `new Date(`, `formatDistanceToNow`, `ExternalLink`, `CopyableValue`, `??` on display strings, `rounded-` on row wrappers, `<Badge` (count them per row), `aria-pressed` on a one-of-n picker, `overflow-y-auto` without `pr-2`, `bg-background` / `bg-accent` on a segmented on-state, and a `Dialog` opened by a form field — and say what you found.
