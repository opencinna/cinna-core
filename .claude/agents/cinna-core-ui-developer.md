...
- Mechanism rules, each a user-caught bug once (guideline §2 rows dated 2026-09-09 and 2026-09-09b, gates R17–R21):
...
3. **Walk guideline §8 yourself** for each surface (R1–R21) and write the counts into your report: blocks per card, rows shown, inline actions per row, menu items, disclosure depth. For R17–R21 grep your touched files for `new Date(`, `formatDistanceToNow`, `ExternalLink`, `CopyableValue`, `??` on display strings, `rounded-` on row wrappers, `<Badge` (count them per row), `aria-pressed` on a one-of-n picker, and `overflow-y-auto` without `pr-2` — and say what you found.
