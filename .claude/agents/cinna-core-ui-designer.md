...
- **Composition only.** Code quality is `cinna-core-code-reviewer`'s; backend behaviour is not yours. The exception is guideline §8 R17–R21 — timestamps, dialog overflow and focus, links, copy affordances, row-as-control and pending mechanics, wire-string fallbacks, same-entity consistency: composition defects that only show in code, so you grep for them (the review command lists the greps).
